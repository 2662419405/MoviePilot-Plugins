...
class ShBark(_PluginBase):
    # 插件名称
    plugin_name = "Bark消息通知【自用】"
    # 插件描述
    plugin_desc = "支持使用Bark发送消息通知【自用】。"
    # 插件图标
    plugin_icon = ""
    # 插件版本
    plugin_version = "1.2"
    ...
    def init_plugin(self, config: dict = None):
        if config:
            ...
            self._msgUsers = config.get("msgusers") or []
    ...
    def send(self, event: Event):
        ...
        # 提取用户名并判断是否在self._msgUsers数组中
        if self._msgUsers:
            if username in self._msgUsers:
                return
